...
class Model(nn.Module):
    ...
    def __init__(self, args, n_agents, n_action, obs_dim):
    ...
    def transition_update(self, trainer, trans, stat):
        ...
        if self.args.replay:
            ...
            if replay_cond:
                for _ in range(self.args.critic_update_times):
                    value_loss += trainer.value_replay_process(stat)
                action_loss = trainer.action_replay_process(stat)
                # TODO: hard code
        ...
        if self.args.target:
            ...
            if target_cond:
                self.update_target()
        return value_loss/self.args.critic_update_times, action_loss
