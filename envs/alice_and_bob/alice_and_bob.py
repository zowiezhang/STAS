...
class Alice_and_Bob(object):
    def __init__(self, map_size=(9, 9), n=2):
        ...
        self.agent_num = n
        self.obs_dim = 19
        self.state_dim = self.length*self.width
        self.n_action = 4
        self.generate_map()
    ...
    def step(self, action_list):
        ...
        #     reward += 10
            # done = True
        # elif self.agt_pos[0] not in self.room_area or self.agt_pos[1] not in self.room_area:
        #     reward = reward + 0.5
        ...
        # check treasure
        if (self.agt_pos[0] == self.treasure_pos or self.agt_pos[1] == self.treasure_pos) and \
            (self.agt_pos[0] not in self.room_area and self.agt_pos[1] not in self.room_area):
            reward = reward + 100
        ...
        reward = [reward for i in range(self.agent_num)]
        done = [done for i in range(self.agent_num)]
