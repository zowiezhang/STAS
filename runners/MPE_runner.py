...
class Runner_QMIX_MPE:
    ...
    def run(self, ):
        ...
        for i_episode in range(self.args.num_episodes):
            ...
            for i in range(self.args.revision_factor):
                episode_reward, _ = self.run_episode_MPE(evaluate=False)  # Run an episode
                self.episode += 1
    ...
    def evaluate_policy(self):
        ...
        if self.args.wandb:
            wandb.log({'agent_reward': evaluate_rewards / self.args.num_eval_runs}, step=self.episode)
        else:
            print('evaluate policy, agent reward: {:.4f}'.format(evaluate_rewards / self.args.num_eval_runs))
...
class Runner_STAS_MPE:
    ...
    def run(self):
        ...
        for i_episode in range(self.args.num_episodes):
            ...
            while num_steps < self.args.batch_size:
                episode_reward, t, = self.run_episode_MPE(evaluate=False, i_episode=i_episode, ep_num=ep_num)
    ...
    def run_episode_MPE(self, evaluate, i_episode, ep_num):
        ...
        return episode_reward, t, # action_e, info_e

    def evaluate_policy(self, i_episode):
        evaluate_rewards = 0

        for _ in range(self.args.num_eval_runs):
            episode_reward, t, = self.run_episode_MPE(evaluate=True, i_episode=i_episode, ep_num=_)
            evaluate_rewards += episode_reward
...
class Runner_SQDDPG_MPE(object):
    ...
    def __init__(self, args, env_name, env, n_agents, n_action, obs_dim, device):
        ...
        # self.logger = Logger(args.tensorboard_save_path)

        self.n_agents = n_agents
        self.n_action = n_action
        self.obs_dim = obs_dim

        self.args.agent_num = n_agents
        self.args.action_dim = n_action
        self.args.obs_size = obs_dim
    ...
    def run(self, ):
        ...
        for i_episode in range(self.args.num_episodes):
            if self.episode // self.args.eval_freq > evaluate_num:
            # if i_episode % self.args.eval_freq == 0:
                self.evaluate_policy(stat)  # Evaluate the policy every 'evaluate_freq' steps
                evaluate_num += 1

            for i in range(self.args.revision_factor):
                episode_reward, _ = self.run_episode_MPE(stat, evaluate=False)  # Run an episode
                # self.logging(stat)
                self.entr += self.entr_inc
                if not self.args.wandb:
                    print('train episode reward: ', episode_reward, ', episode: {}'.format(self.episode))

        self.evaluate_policy(stat)

    def run_episode_MPE(self, stat, evaluate=False):
        info = {}
        episode_reward = 0
        ...
        for t in range(self.args.max_step_per_round):
            ...
            if not evaluate:
                if done_:
                    reward = [episode_reward for na in range(self.n_agents)]
                else:
                    reward = [0.0 for na in range(self.n_agents)]
        ...
        if not evaluate:
            if not self.args.wandb:
                ...
                wandb.log({'SQDDPG value loss': np.mean(value_losses), 'SQDDPG action loss': np.mean(action_losses), 'SQDDPG mean reward': stat['mean_reward']}, step = self.episode)
            stat['turn'] = t+1
            self.episode += 1
    ...
    def evaluate_policy(self, stat):
        # win_times = 0
        evaluate_rewards = 0
        for _ in range(self.args.num_eval_runs):
            episode_reward, _ = self.run_episode_MPE(stat, evaluate=True)
            evaluate_rewards += episode_reward
